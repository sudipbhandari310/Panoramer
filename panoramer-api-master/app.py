...
def generate_panorama():
    try:
        ...
        # sorting files, extracting numeric parts, fallback to original filename if no digits
        uploaded_files = sorted(uploaded_files, key=lambda x: ''.join(filter(str.isdigit, Path(x).stem)) or x)
        ...
        logging.error(f"Error during panorama generation: {str(e)}")
        return jsonify({"message": str(e)}), 500
...
def delete_uploads():
    try:
        ...
        logging.error(f"Error during uploads clearing: {str(e)}")
        return jsonify({"message": str(e)}), 500
...
def serve_all_files():
    try:
        ...
        if not os.path.exists(results_folder):
            return jsonify({"message": "No files available!"}), 400
        ...
        logging.error(f"Error during file serving: {str(e)}")
        return jsonify({"message": str(e)}), 500
